...
def add_p(PARAMETER_BASE, i, parameter):
...
def target_similarity(A,metric,jtfs_x=None):
    ...
    match metric:
        ...
        case "mfcc":
            A = mfcc(A)
            return mfcc_distance(A, TARGET_MFCC)
        case "jtfs":
            A = extract_jtfs(A,worker_jtfs)
            return jtfs_distance(A,TARGET_JTFS)
        case "dtw":
            A = mfcc_raw(A).T
            return dtw_distance(A, TARGET_MFCC_RAW)
...
def dtw_distance(mfccA, mfccB):



    distance = dtw(specA,specB,global_constraint="sakoe_chiba",sakoe_chiba_radius=10)
    print(distance)
    return distance
...
target = "target_1"
main_synth.load_preset("targets/"+target+".vital")

TARGET_AUDIO = render_synth(main_synth)
TARGET_SPEC = create_mel_spec(TARGET_AUDIO)
TARGET_MFCC = mfcc(TARGET_AUDIO)
TARGET_MFCC_RAW = mfcc_raw(TARGET_AUDIO)
...
graph_spec(TARGET_SPEC, target)

wavfile.write("audio/"+target,SAMPLE_RATE,TARGET_AUDIO.T)
